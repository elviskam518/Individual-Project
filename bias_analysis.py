...
def run_shap_analysis(df, baseline_group="Male_White", out_dir="."):
    ...
    results_df.to_csv(os.path.join(out_dir, "shap_bias_analysis.csv"), index=False)
    summary.to_csv(os.path.join(out_dir, "shap_feature_summary.csv"))
    
    try:
        plt.figure(figsize=(10, 6))
        shap.summary_plot(shap_values, X_scaled, feature_names=feature_cols, show=False)
        plt.savefig(os.path.join(out_dir, "shap_summary_plot.png"), bbox_inches='tight', dpi=150)
        plt.close()
        
        print("\nSHAP plots saved: shap_summary_plot.png")
    except Exception as e:
        import traceback
        print(f"\nFailed to generate SHAP plots: {e}")
        traceback.print_exc()
