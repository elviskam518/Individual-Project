...
def train_baseline(model, data, epochs=150, batch_size=256, lr=0.001, device="cpu", verbose=True):
    model = model.to(device)
...
def run_experiment(csv_path="tech_diversity_hiring_data.csv"):
    ...
    print(
    f"Baseline TEST Acc={base_test['accuracy']:.4f}, "
    f"F1={base_test['f1']:.4f}"
    )
    ...
    data_clean = {
        "X_train": data["X_train"][:, clean_idx],
        "X_val": data["X_val"][:, clean_idx],
        "X_test": data["X_test"][:, clean_idx],
    ...
    train_baseline(base_clean, data_clean, epochs=150, batch_size=256, lr=1e-3, device=device, verbose=False)
    _ = evaluate_baseline(base_clean, data_clean, device=device)
